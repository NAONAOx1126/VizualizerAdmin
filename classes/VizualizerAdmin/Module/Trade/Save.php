 ...
 * @author Naohisa Minagawa <info@vizualizer.jp>
 */
class VizualizerAdmin_Module_Trade_Save extends Vizualizer_Plugin_Module_Save
{
            ...
            try {
                ...
                if (!empty($post[$this->key_prefix . $primary_key])) {
                    ...
                    if (!($model->$primary_key > 0)) {
                        $model = $loader->loadModel("Trade", array($primary_key => $post[$this->key_prefix . $primary_key]));
                    }
                }
                ...
                foreach($post["details"] as $detail){
                    if(!empty($detail["trade_detail_name"]) && is_numeric($detail["price"]) && $detail["quantity"] > 0){
                        $inputs[] = $detail;
                    }
                }
                ...
                if ($index <= count($inputs)) {
                    ...
                    for ($i = $index; $i < count($inputs); $i ++) {
                        $model = $loader->loadModel("TradeDetail");
                        $model->trade_id = $trade->trade_id;
